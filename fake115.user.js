...
(function() {
    'use strict';
var LoginEncrypt_, browserInterface, browser_115_m115_decode, browser_115_m115_encode, bytesToString, dictToQuery, g_checksum, g_kts, m115_asym_decode, m115_asym_encode, m115_getkey, m115_sym_decode, m115_sym_encode, prsa, srsa, stringToBytes, sysinfo_checksum, xor115_enc, _ref;
...
sysinfo_checksum = function(src) {
  var i, x, _i, _ref;
  x = 0;
  for (i = _i = 0, _ref = src.length; _i < _ref; i = _i += 2) {
    x = g_checksum[x ^ parseInt(src.slice(i, i + 2), 16)];
  }
  x ^= 0x55;
...
LoginEncrypt_ = function(_arg, g) {
  var account, data, environment, fake, goto, key, nonce, passwd, tm, tmus, token, _ref;
  ...
  _ref = browser_115_m115_encode(JSON.stringify({
    ...
    passwd: passwd,
    sign: md5("" + account + tm),
    system_info: sysinfo_checksum("" + fake + fake.slice(0, 16)).toUpperCase(),
    time: tm
